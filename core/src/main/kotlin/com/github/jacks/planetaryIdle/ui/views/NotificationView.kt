...
class NotificationView(skin : Skin) : KTable, Table(skin) {
            ...
            skin.add(PIXMAP_KEY, TextureRegionDrawable(
                Texture(
                    Pixmap(1, 1, Pixmap.Format.RGBA8888).apply {
                        this.drawPixel(0, 0, Color.rgba8888(0.23f, 0.23f, 0.23f, 0.3f))
                    }
                )
            ))
        }
...
) : NotificationView = actor(NotificationView(skin), init)
